    ...
    public function actionIndex() {
        ...
        <HTML>
            <HEAD>
                ...
                <style>  @viewport {  width: device-width;  zoom: 1.0; user-zoom: fixed ; orientation: portrait;  }</style>
  
            </HEAD>
            <?php 
                    if( !$log || !$idGamer ){
                        echo "<h1>Для участия в игре необходима авторизация.</h1>";
                        echo "<p>Перейдите в раздел <a href='/site/login'>регистрации/авторизации.</a></p>";
                        exit;
                    }
                ?> 
            <BODY>              
                <div id="YMapsID" style="  height: 100vh; position: relative; margin: 0 auto;  ">
                     <div id="informStr" style="position: absolute; left: 10px; top: 5px; z-index: 1000; padding: 5px; width: 70vw; background: black; color: white; font: 40px/50px arial; text-align: center;"><?php echo $strStartMess ?></div>
                     <BUTTON type="button" id="gameControl" style=" position: absolute; right: 10px; top: 10px; z-index: 1000; padding: 5px;width: 25vw; height: 10vh; cursor: pointer; background: #FF7F50; font-size: 40px; ">Start/Stop</BUTTON>
